 ...
 * Get a list of CSS selectors of nodes that have an implicit role
 * @param  {String} role The role to check
 * @return {Boolean}       Either an Array of CSS selectors or `null` if there are none
 */
aria.childrenArePresentational = function (role) {
	var roles = lookupTables.role;
	for (role in roles) {
		if (roles.hasOwnProperty(role)) {
			return !!roles[role].childrenArePresentational;
		}
	}
